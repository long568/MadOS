...
uint16_t MAX30105::check(void)
{
    ...
    if (readPointer != writePointer)
    {
        ...
        dat = (char*)malloc(bytesLeftToRead);
        if(!dat) {
            goto END_CHECK;
        }
        ...
        buf[0] = MAX30105_READ;
        buf[1] = MAX30105_FIFODATA;
        buf[2] = bytesLeftToRead;
        write(_i2cdev, buf, 3);
        rc = read(_i2cdev, dat, bytesLeftToRead);
        if(rc < 0) {
            goto END_CHECK;
        }
