    ...
    do {
        ...
        if(0 == TransferLocker)
            while(1)
                __asm volatile ("NOP");       
        NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
        NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = ISR_PRIO_MicroSD;
        NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
        NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_WriteBlock(const uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize)
{
  SD_Error errorstatus = SD_OK;
...
SD_Error SD_WaitAnyOperation(void)
{
    madSemWait(&TransferLocker, 0);
    if(SD_OK == TransferError) {
        if (StopCondition == 1) {
            SDIO->ARG = 0x0;
            SDIO->CMD = 0x44C;
...
{  
    if(SET == SDIO_GetITStatus(SDIO_IT_DATAEND)) {
        TransferError = SD_OK;
    } else {
        CmdStatus = SDIO->STA;
    }
    
    SDIO_ClearITPendingBit(0x00FFFFFF);
    SDIO_ITConfig(0x00FFFFFF, DISABLE);
    madSemRelease(&TransferLocker);
    return(TransferError);
}
...
//  }
    ...
    if ((MAD_ERR_TIMEOUT == error) || (CmdStatus & SDIO_FLAG_CTIMEOUT))
    {
...
//        return(errorstatus);
    ...
    if (CmdStatus & SDIO_FLAG_CMDREND)
    {
...
//  return(errorstatus);
    ...
    SDIO_ITConfig(SDIO_IT_CCRCFAIL | SDIO_IT_CMDREND | SDIO_IT_CTIMEOUT, ENABLE);
    madSemWait(&TransferLocker, 0);
    if(CmdStatus & SDIO_FLAG_CTIMEOUT)
        return SD_CMD_RSP_TIMEOUT;
    return SD_OK;
}
...
//  return(errorstatus);
    ...
    if(CmdStatus & SDIO_FLAG_CTIMEOUT) {
        return SD_CMD_RSP_TIMEOUT;
    } else if(CmdStatus & SDIO_FLAG_CCRCFAIL) {
